...
在正式了解`HTTPS`协议之前，先简单介绍一下两个重要的概念：

- 对称加密：加密和解密使用相同的秘钥
- 非对称加密：加密使用的钥匙和解密使用的钥匙不一样

由于使用**对称加密**使用同样的秘钥，**非对称加密**使用不同的秘钥，所以对称加密拥有更高的效率以及更好的性能。

这里我们可以思考一下：**既然对称加密这么好用，是不是我们只需使用对称加密就可以完成`HTTPS`协议了?**
...
现在，我们可以回答一下上面的问题了，仅仅使用**对称加密**显然是实现不了数据的安全传输的，因为这样秘钥在网络中很容易被窃取，在没有加密的前提下。因此在实际的HTTPS协议中，非对称加密用来传输对称加密的秘钥，而对称加密则用来传输数据。

因此，HTTPS的过程大概是：

1. 客户端发送一个消息，包括`TLS`版本、加密套件候选列表、压缩算法候选列表。以及一个随机数。
2. 服务器返回一个消息，包括协议、加密套件、压缩算法。以及一个随机数。
3. 客服端选择加密等信息，发送给服务端。
4. 服务器下发证书，客户端验证。
5. 验证成功后，客户端随机数 + 服务端随机数 + pre-Master随机数 生成对称加密的秘钥。
6. 双方利用对称钥匙进行数据的通信。

*图片来自于[《趣谈网络协议》](https://time.geekbang.org/column/intro/85)*）

![HTTPS过程](https://teaof-konwleadge-1255982134.cos.ap-shanghai.myqcloud.com/blog/%E7%BD%91%E7%BB%9C/%E7%BD%91%E7%BB%9C%E5%9F%BA%E7%A1%80/HTTPS%E5%8E%9F%E7%90%86.jpg)
